...
> Estado de continuidad al **2026-08-18**. La auditoría de release quedó consolidada localmente en la rama `dev`; el segundo rerun remoto confirmó que el respawn POSIX necesitaba basarse en el estado del worker y esa corrección ya está aplicada.
...
- Último commit de código local: `4a8ca8e217 fix: make POSIX worker respawn state driven`; hace que el respawn use los estados `unhealthy/stopped`, además de que `build:desktop` construya las dependencias del workspace antes de Electron. Está listo para empujar a `origin/dev` junto con la actualización de este estado.
...
1. Empujar `4a8ca8e217` y esta actualización a `dev`, recrear/empujar `v0.1.0` y verificar el workflow multiplataforma completo.
