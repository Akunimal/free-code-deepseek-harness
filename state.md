...
> Estado de continuidad al **2026-08-17**. La auditoría de release quedó consolidada localmente en la rama `dev`; el actualizador, el renombre del fork y la documentación bilingüe están listos para la validación final.
...
- El último commit de código publicado fue `4a8ca8e217 fix: make POSIX worker respawn state driven`; sobre ese estado quedan locales el actualizador, el rename/documentation cleanup y la nueva validación final.
...
- `apps/shell/electron-builder.yml` genera `com.freecode.deepseekharness` con `extraResources`, asar, icono, instalador NSIS y `.exe` portable, y publica en el fork `Akunimal/free-code-deepseek-harness` como GitHub draft.
...
- `pnpm test`: **3 paquetes, 13 archivos de test, 48 tests verdes** (9 adapter, 13 contracts, 26 shell); prepara automáticamente los 234 enlaces de paquetes upstream que necesita el loader dinámico. El adapter incluye una verificación real de respawn tras `SIGKILL`; shell cubre descarga confirmada y detección de commits upstream.
...
1. Commit/push de esta actualización a `dev`, recrear/empujar `v0.1.0` y verificar el workflow multiplataforma completo y sus assets.
