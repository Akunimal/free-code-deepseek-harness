...
The current local patch release target is `v0.1.7`. The Windows artifacts were rebuilt locally on 2026-08-22 and validated with a silent per-user install. Before tagging or publishing, verify the full local preflight: zero-config boot, model discovery, provider registration, message streaming, worker restart, tray/overlay actions, import/continuation, workspace lifecycle, update checking, headless tool execution, and the conversation motion layer.
