...
- `ctx.workspaceRegistry.delete(id)`：移除 Workspace 注册记录及其持久顺序条目，并将其已记账会话加入注册表级归档集合。未知 id 返回 `false`，成功移除记录则返回 `true`。目录、用户文件、活跃会话和持久化会话日志绝不受影响；已归档会话不会进入任何分组视图，也不会变成 Ungrouped。表写入失败时会恢复原顺序、归档集合和此前发布的实体。
