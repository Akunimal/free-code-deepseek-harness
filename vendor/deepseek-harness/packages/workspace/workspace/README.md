...
- `ctx.workspaceRegistry.delete(id)` — removes the Workspace registration and its durable order entry, and archives its accounted Sessions in the registry-global archive set. Unknown ids return `false`; a removed record returns `true`. The directory, user files, live Sessions, and persisted session logs are never touched; archived Sessions stay out of grouping surfaces instead of becoming Ungrouped. A table-write failure restores the prior order, archive set, and published entity.
