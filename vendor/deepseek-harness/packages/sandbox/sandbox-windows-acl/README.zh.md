...
- **控制台创建会被隐藏，而不是被禁用。** 在受限令牌下，`CREATE_NO_WINDOW` / `CREATE_NEW_CONSOLE` 可能使子进程在 DLL 初始化期间以 `STATUS_DLL_INIT_FAILED`（`0xC0000142`）退出。因此 runner 保持这些创建标志为零，传递 `STARTF_USESHOWWINDOW` 与 `SW_HIDE`，并通过管道重定向 stdio；这样保留受限令牌行为且不显示控制台窗口。
