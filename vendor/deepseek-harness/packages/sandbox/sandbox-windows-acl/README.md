...
- **Console creation is hidden rather than disabled.** Under the restricted token, `CREATE_NO_WINDOW` / `CREATE_NEW_CONSOLE` can kill children during DLL initialization with `STATUS_DLL_INIT_FAILED` (`0xC0000142`). The runner therefore keeps those creation flags clear, passes `STARTF_USESHOWWINDOW` with `SW_HIDE`, and redirects stdio through pipes. This preserves the restricted-token behavior without showing a console window.
