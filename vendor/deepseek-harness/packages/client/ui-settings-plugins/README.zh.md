...
本包自带的卡片覆盖 shell 执行器（`bash`）、agent 循环的工具调用并行度（`agent-loop`）以及 DeepSeek 搜索提供方（`web-search-deepseek`）。Shell 卡片包含 RTK 压缩开关，默认开启，但只使用用户已经安装的 RTK；未找到 RTK 时命令保持不变。
...
- **shell 卡片跟随被组装的执行器**——POSIX 与 PowerShell 两个执行器家族共用 `bash` 命名空间，因为一个宿主只组装其中之一，所以被服务的 schema 随平台不同（PowerShell 多出 `pwshPath`），尽管卡片在两者下编辑的都是同样的用户字段；而两者都不组装的部署不会显示这张卡片。
