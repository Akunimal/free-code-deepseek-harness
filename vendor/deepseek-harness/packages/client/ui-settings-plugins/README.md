...
The cards this package ships cover the shell executor (`bash`), the agent loop's tool-call parallelism (`agent-loop`), and the DeepSeek search provider (`web-search-deepseek`). The shell card includes an RTK compression toggle; it defaults on, but only uses an RTK executable already installed by the user and leaves commands unchanged when RTK is absent.
